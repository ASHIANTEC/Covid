...
app.post('/login',(req,res) =>{
    ...
    var sql = `SELECT password FROM covid.login`;
    conn.query(sql,(err,result)=>{
     ...
     else if(password == result[1].password){
            message="";
            message="Login Successfully!"
            res.render('form1.ejs',{message:message,userData:userData});
            console.log('ok');
        }
        else{
            message="";
...
app.post('/login1',(req,res) =>{
    var message ='';
    var password = req.body.password;
    var sql = `SELECT *  FROM covid.login WHERE (id = 2)`;
    conn.query(sql,(err,result)=>{
        // console.log("result",result[0].password);
        if(password == result[0].password){
            message="";
                        ...
                        conn.query(sql,(err,data)=>{                
                            ...
                            var sql = "INSERT INTO `covid`.`medicine`(`patient_id`,`ct_scan_id`,`ct_scan_centre`,`pcr_icmr`,`receipt_no`,`date`) VALUES('"+patient_id+"','"+ct_scan_id+"','"+ct_scan_centre+"','"+pcr_icmr+"','"+receipt_no+"','"+today+"')";
                                conn.query(sql,(err,data)=>{   
                                    ...
                                    // message = "Successfully Submitted!";
                                    // // res.render(__dirname +'form.html',{ message, userData: data });
                                    // res.render('view/form.ejs',{message:message});
